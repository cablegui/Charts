	...
	public class UsingLabels : System.Windows.Forms.UserControl
	{
		...
		private void Labels()
		{
			...
			foreach(DataPointCustomProperties attrib in Chart1.Series)
			{
				...
				// Set labels Position
				attrib["LabelStyle"] = LabelPosition.GetItemText(LabelPosition.SelectedItem);

				// Set labels font
				attrib.Font = new Font(FontNameList.GetItemText(FontNameList.SelectedItem), int.Parse(FontNameSize.GetItemText(FontNameSize.SelectedItem)), FontStyle.Bold);

				// Set labels angle
				attrib.LabelAngle = int.Parse(AngleList.GetItemText(AngleList.SelectedItem));

				// Set labels color
				attrib.LabelForeColor = Color.FromName(FontColorCombo.GetItemText(FontColorCombo.SelectedItem));

				// Set labels background color
				attrib.LabelBackColor = Color.FromName(FontColorCombo.GetItemText(comboBoxBackColor.SelectedItem));

				// Set labels border color
				attrib.LabelBorderColor = Color.FromName(FontColorCombo.GetItemText(BorderColor.SelectedItem));

				// Set labels border width
				attrib.LabelBorderWidth = int.Parse((string)BorderSize.SelectedItem);
			}
		}
